...
LastEditTime: 2026-06-24 10:05:52
'''
...
def gen_parquet(wav_dir, parquet_dir, mid_name='', file_type='wav', num_utts_per_parquet=2000, num_processes=5, process_max_num=10000, use_pre_dir=False, pre_dir='merge'):
        ...
        for utt in tqdm(uttlist[process_max_num * tid : process_max_num * (tid+1)], desc=f'Turn{str(tid)}LoadAudio'):
            ...
            if file_type == 'wav':
                ...
                try:
                    ...
                    print(f"⚠️ 文件格式错误或损坏，跳过: {wavpath} | 错误: {e}")
                    continue
